...
appengine_dirs = ['/home/administrator/workspace/google_appengine','/home/administrator/workspace/google_appengine/lib','/home/administrator/workspace/google_appengine/lib/yaml','/home/administrator/workspace/google_appengine/lib/antlr3',]
...
PROJECT_ROOT='/home/administrator/workspace/my_cms/'
...
firstpost=db.GqlQuery("SELECT * FROM Post ORDER BY date").get()    

now=firstpost.date
while(now<=datetime.datetime.now()):
    firstday=datetime.datetime(now.year,now.month,1)

    if now.month!=12:     
        lastday=datetime.datetime(now.year,now.month+1,1)
    else:
        lastday=datetime.datetime(now.year+1,1,1)
   
    entrycount=db.GqlQuery("SELECT * FROM Post WHERE date > :1 AND date <:2 ORDER BY date DESC",firstday,lastday).count()                           
    if entrycount:
        archive = Archive.get_or_insert(
                                                key_name='a'+firstday.strftime('%B %Y'),
                                                name=firstday.strftime('%B %Y'),

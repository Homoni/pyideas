...
part1 = open(ROOT_PATH+'/daodao2.JPG.part1',"w")
part1.write(file.read(3662))
response = client.post('http://localhost:8080/upload/album/', {'file': part1},HTTP_RANGE='bytes=%s-%s' % (0, 3662))
part1.close()

part2 = open(ROOT_PATH+'/daodao2.JPG.part2',"w")
file.seek(3663)
part2.write(file.read())
response = client.post('http://localhost:8080/upload/album/', {'file': part2},HTTP_RANGE='bytes=%s-%s' % (3663, size))
part2.close()
